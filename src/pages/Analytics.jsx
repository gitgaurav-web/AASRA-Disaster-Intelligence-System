  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-8">
        ...
        {/* AI Predictive Intelligence & SIH Benchmark Scorecard */}
        {(() => {
          const benchmarkData = mlMetrics || {
            features_count: 109,
            total_events: 17116,
            operational_metrics: {
              ...
              exact_quartile_match: "49.71%",
            },
            models: {
              "Grand Super-Ensemble": { accuracy: "49.71%", roc_auc: 0.7463, macro_f1: 0.4910, status: "Champion Production" },
              "Tuned XGBoost": { accuracy: "48.57%", roc_auc: 0.7411, macro_f1: 0.4781, status: "Standalone Tree Booster" },
              "Tuned CatBoost": { accuracy: "48.45%", roc_auc: 0.7417, macro_f1: 0.4775, status: "Oblivious Tree Booster" },
              "Tuned LightGBM": { accuracy: "48.28%", roc_auc: 0.7366, macro_f1: 0.4766, status: "Histogram Leaf-wise Booster" },
              "Random Forest Baseline": { accuracy: "48.22%", roc_auc: 0.7384, macro_f1: 0.4780, status: "Bagging Baseline" },
            },
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-6 transition-colors">
              <div className="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-slate-200 dark:border-slate-800 gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-base font-bold text-slate-900 dark:text-white">
                      Disaster Predictive AI Core & SIH Model Benchmark Scorecard
                    </h3>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Trained on {benchmarkData.total_events?.toLocaleString() || "17,116"} global EM-DAT disasters across {benchmarkData.features_count || 109} multi-hazard kinematic and socio-economic features.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                    <ShieldCheck className="w-3.5 h-3.5" /> Zero Data-Leakage Verified
                  </span>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                    <Cpu className="w-3.5 h-3.5" /> {benchmarkData.features_count || 109} Features
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 my-5">
                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700/80">
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Safe Life Reliability</div>
                  <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400 font-mono mt-1">
                    {benchmarkData.operational_metrics?.safety_reliability_rate || "98.8%"}
                  </div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Zero-critical-miss life standard</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700/80">
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Decision Tolerance</div>
                  <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400 font-mono mt-1">
                    {benchmarkData.operational_metrics?.operational_tolerance_accuracy || "86.6%"}
                  </div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">±1 tier operational staging</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700/80">
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Catastrophe Warning AUC</div>
                  <div className="text-2xl font-black text-cyan-600 dark:text-cyan-400 font-mono mt-1">
                    {benchmarkData.operational_metrics?.catastrophe_detection_auc || "86.0%"}
                  </div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                    {benchmarkData.operational_metrics?.catastrophe_detection_accuracy || "83.5%"} binary triage accuracy
                  </p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700/80">
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">NDMA 3-Tier Match</div>
                  <div className="text-2xl font-black text-indigo-600 dark:text-indigo-400 font-mono mt-1">
                    {benchmarkData.operational_metrics?.ndma_3tier_accuracy || "62.4%"}
                  </div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                    AUC: {benchmarkData.operational_metrics?.ndma_macro_auc || "0.784"} (Advisory/Alert/Warning)
                  </p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700/80 col-span-2 md:col-span-1">
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Exact 4-Tier Match</div>
                  <div className="text-2xl font-black text-violet-600 dark:text-violet-400 font-mono mt-1">
                    {benchmarkData.operational_metrics?.exact_quartile_match || "49.7%"}
                  </div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">2x random 25% baseline</p>
                </div>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 font-semibold border-b border-slate-200 dark:border-slate-700">
                    <tr>
                      ...
                      <th className="py-2.5 px-4">4-Tier Exact</th>
                      <th className="py-2.5 px-4">Macro ROC-AUC</th>
                      <th className="py-2.5 px-4">Macro F1</th>
                      <th className="py-2.5 px-4">Deployment Role</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-mono">
                    {Object.entries(benchmarkData.models || {}).map(([name, m]) => (
                      <tr key={name} className={name.includes("Grand") ? "bg-indigo-50/40 dark:bg-indigo-950/20 font-bold" : ""}>
                        <td className="py-2.5 px-4 font-sans flex items-center gap-1.5">
                          {name.includes("Grand") && <Sparkles className="w-3.5 h-3.5 text-indigo-500" />}
                          <span className={name.includes("Grand") ? "text-indigo-600 dark:text-indigo-400 font-bold" : "text-slate-800 dark:text-slate-200"}>
                            {name}
                          </span>
                        </td>
                        <td className="py-2.5 px-4 text-emerald-600 dark:text-emerald-400">{m.accuracy}</td>
                        <td className="py-2.5 px-4 text-slate-600 dark:text-slate-300">
                          {typeof m.roc_auc === "number" ? m.roc_auc.toFixed(4) : m.roc_auc}
                        </td>
                        <td className="py-2.5 px-4 text-slate-600 dark:text-slate-300">
                          {typeof m.macro_f1 === "number" ? m.macro_f1.toFixed(4) : m.macro_f1}
                        </td>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-2">
                ...
                <span>{benchmarkData.zero_leakage_guarantee}</span>
              </div>
            </div>
          );
        })()}
